...
var gettxy =  function (e) {
    ...
    var resizegame = function () {
        ...
        if (bw*1.2>bh) {canvas.height = bh; 
                        canvas.width = Math.floor(bh * 0.6);} 
                else   {canvas.width  = bw; 
                        canvas.height = Math.floor(bw * 1.2)};
...
//board && pieces - main game assets - mian interact functions
    ...
    var Gameassets = function () {
        ...
        var Board = function () {
            function Board(level) {
                ...
                //this.piecons = [[2,1000]];
                this.pc = [];
                while (this.piecons.length>0){while (this.piecons[0][1]>0){this.pc.push(this.piecons[0][0]);this.piecons[0][1] = this.piecons[0][1]-1}; this.piecons.shift();};
                for (var value = this.level; value > 0; value--) {
                        if (this.pc.length>0) {this.pc.shift();} else {return this.pc = [2, 3, 4];};
                    };
                ...
                var tryconstruct = function (pc) {
                        ...
                        //use sizemap && placemap to construct basic pieces!! FTW!! pm = placemap sm = sizemap
                            var piecearraymaker = function (pm, sm) {
                                var  checkit = function (arra, element) {
                                                    if (arra.length == 0) {return true};
                                                    for (i=0; i < arra.length; i ++) {
                                                        if (
                                                            ((arra[i] == element + 1) && (element % 6 != 6-1)) ||
                                                            ((arra[i] == element - 1) && (element % 6 != 0)) ||
                                                            (arra[i] == element - 6) ||
                                                            (arra[i] == element + 6))  {return true};
                                                        };
                                                    return false;
                                                };
                                ...
                                while (sm.length>0) {
                                      ...
                                      places = [];
                                      while (len > 0) {
                                          if (pushitorleaveit(places, pm)) {len = len-1
                                            } else {
                                                sm.push(len);
                                                len = 0;
                                               };
                ...
                //gettingridof pieces which are to long for interface to look good
                 ...
                 while (pa[0].length >= 5) {
                     var p = pa[0];
                     var f1 = function (a) {return Math.floor(a / 6);};
                     var f2 = function (a) {return a % 6;};
                     if (   (pa[0].length>5) 
                         || (f1(p[0]) == f1(p[1])== f1(p[2])== f1(p[3])== f1(p[4]))
                         || (f2(p[0]) == f2(p[1])== f2(p[2])== f2(p[3])== f2(p[4]))){pa = tryconstruct(this.pc);} else {pa.push(pa.shift());}    
                 };
            ...
            //very important function - produces piece out of array val and an array of elements (fex selected)
             ...
             Board.prototype.deselectall = function () {
                    while(this.selected.length>0) {this.squares[this.selected.pop()].deselect();};
                };
             Board.prototype.checkdestruction = function () {
                    
                    for (var val = 0; val < 36; val++) {
                        if (this.squares[val].data.value>0) {return false;} };
                    this.deselectall();
                    return true;
                };
             Board.prototype.destruct = function () {
                     while(this.selected.length>0) {var s = this.squares[this.selected.pop()];
                                                    s.data.value = 0;
                                                    s.break();};
                     if (this.checkdestruction()) {setTimeout(GameStatusChanger.nextlevel, 450);};
        ...
        var PieceBox = function () {
            ...
            PieceBox.prototype.selecto = function () {
                    for (var v = 0; v<this.selected.length; v++) {this.pa[this.selected[v]].selecto();
                                                                  this.draw(this.selected[v]);};
                };
            PieceBox.prototype.deselect = function () {
                    for (var v = 0; v<this.pa.length; v++) {this.pa[v].deselect();};
                    this.selected = [];
                };
            ...
            PieceBox.prototype.nextl = function () {
                    console.log('yy');
                    for (var v = 0; v<this.pa.length; v++) {this.pa[v].nextl();};        
                };
            PieceBox.prototype.draw = function (nu) {
                    this.pa[nu].draw();
                };
            PieceBox.prototype.drawall = function () {
                    for (var v = 0; v<this.pa.length; v++) {this.pa[v].draw();};
                };
            PieceBox.prototype.checkout = function (npece) {
                   this.deselect();
                   for (var v = 0; v<this.pa.length; v++) {if ((this.pa[v].fresh) && (npece.checkout(this.pa[v]))) {var z = v; this.selected.push(z)}};
                   if (this.selected.length>0) {this.selecto();
                                                return true;} else {this.deselect();
                                                                    return false;};
        ...
        function Gameassets (level) {
            ...
            var checkit = function () {return pieb.checkout(boa.a2p());
                                      };
            ...
            var doit = function () {if (checkit()) {GameStatusChanger.score();
                                                    boa.destruct();
                                                    pieb.clear();
                                                 } else {cancelit()};
                                   };
       
            var addelement = function (nu) {boa.selecto(nu);
                                            checkit();};
            
            
            inpu.arr.push(new ActionOb(bsx-rs.x, bsy-rs.x, bsx+bsi*6+rs.x, bsy+bsi*6+rs.x,
                                   function () {doit();},
                                   function () {cancelit();},
                                   function () {}));
           
            var createit = function (num) {inpu.arr.push(new ActionOb(bsx+v%6*bsi, bsy+Math.floor(v/6)*bsi, 
                                                                bsx+v%6*bsi+bsi, bsy+Math.floor(v/6)*bsi+bsi,
                                             function () {},
                                             function () {},
                                             function () {addelement(num);}))};
            for (var v = 0; v<36; v++) {createit(v);
                                         };
    ...
    var Timer = function () {
        ...
        Timer.prototype.start = function (ins) {
                var Tim = this;
                this.countdown = setInterval(function () {if (ins.r1()) {Tim.stop();
                                                                         Tim.gamend()};}, 1000);
                };
    ...
    var NewGame = function () {
        ...
        NewGame.prototype.levelup = function () {
                    ...
                    if (this.i.level == 128) {this.i.level == 'WIN!';
...
var scorealert = function () {if (squaredlines) {
                                var score = squaredlines.i.score;
                                if (score<500) {alert(score+' bricks computed. Next time work harder.');} 
                                                                    else {alert(score+' blocks. You are a computer. Go compute.');}
                              };};


GameStatusChanger.end =     function () {scorealert();
                                         squaredlines.newgame();};

GameStatusChanger.nextlevel = function () {squaredlines.levelup();};
